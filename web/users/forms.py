...
from django.utils.html import format_html, conditional_escape, html_safe
...
class CustomRegistrationForm(RegistrationForm):
    ...
    # def _construct_row(self):
    #     """ Used for custom rows in as_person_details method. """

    def as_person_details(self):
        custom_fields = {
            'user_name': ['first_name', 'last_name', ],
            'address': ['billing_city', 'billing_country_area', 'billing_postcode', ],
        }
        prepared = {}
        for row_label, field_lables in custom_fields.items():
            prepared[row_label] = {'field_names': field_lables}
            prepared[row_label]['fields'] = {name: field for name, field in self.fields.items() if name in field_lables}
        max_columns = 2 * max(len(fields) for fields in prepared.values())
        errors_on_separate_row = False
        help_text_br = False
        row_ender = '</td></tr>'
        ...
        if help_text_br:
            data_col = '%(errors)s<br />%(field)s<br />%(help_text)s'
        else:
            data_col = '%(errors)s<br />%(field)s%(help_text)s'
        ...
        for prep_label, prep_data in prepared.items():
            ...
            for name, field in prep_data['fields'].items():
                ...
                if bf.is_hidden:
                    ...
                    html_class_attr = ' class="%s"' % css_classes if css_classes else None
        ...
        if new_errors:
            ...
            if has_initial_top_errors:
                initial_errors = rows[0][len(error_row_start):-len(row_ender)]
