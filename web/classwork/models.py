...
#         """
#         return self.start_time + timedelta(minutes=self.session.num_minutes)
...
#         end = start + timedelta(days=7*self.session.num_weeks)
#         return (start, end)

#     def name_code(self):
#         """ The truncated code for the class level and version
#         """
#         return f'{self.subject.level}{self.subject.version}'
...
#     def __str__(self):
#         return f'{self.name_code()} {self.session.name}'

#     def __repr__(self):
#         return f'<Class: {self.name_code()} | Session: {self.session.name} | Id: {self.id} >'
